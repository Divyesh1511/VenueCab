...
if ( ! function_exists( 'inspiry_generate_dynamic_css' ) ) {
	...
	function inspiry_generate_dynamic_css( $realhomes_modern_custom_css ) {
		...
		$inspiry_top_menu_gradient_color_1 = get_option( 'inspiry_top_menu_gradient_color_1' );
		$inspiry_top_menu_gradient_color_2 = get_option( 'inspiry_top_menu_gradient_color_2' );
		...
		if ( ! empty( $inspiry_top_menu_gradient_color_1 ) || ! empty( $inspiry_top_menu_gradient_color_2 ) ) {


			if(!empty($inspiry_top_menu_gradient_color_1)){
				$gradient_staring = inspiry_hex_to_rgba( $inspiry_top_menu_gradient_color_1, .7 );
			}else{
				$gradient_staring = 'rgba(0, 0, 0, 0.7)';
			}
			if(!empty($inspiry_top_menu_gradient_color_2)){
				$gradient_ending = inspiry_hex_to_rgba( $inspiry_top_menu_gradient_color_2, 0 );
			}else{
				$gradient_ending = 'rgba(192, 192, 192, 0)';
			}

			$dynamic_css[] = array(
				'elements' => '.rh_header--shadow',
				'property' => 'background',
				'value'    => 'linear-gradient(180deg,' . $gradient_staring . '0%, ' . $gradient_ending . ' 100%);',
			);
